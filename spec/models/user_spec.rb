...
describe User do
  ...
  it "is not saved with a too short password" do
    user = User.create username:"Pekka", password:"Ha4", password_confirmation:"Ha4"
  ...
  it "is not saved with a password containing only letters" do
    user = User.create username:"Pekka", password:"Haahaa", password_confirmation:"Haahaa"
  ...
  describe "favorite brewery" do
    ...
    it "is the one with highest rating if several rated" do
      create_beers_with_ratings(10, 20, 15, 7, 9, user)
      brewery = Brewery.create name:"the best", year:2000
      best = FactoryGirl.create(:beer, brewery:brewery)
      FactoryGirl.create(:rating, score:25, beer:best, user:user)

      expect(user.favorite_brewery.name).to eq("the best")
    end
  end
end
